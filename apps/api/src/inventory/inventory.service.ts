...
export class InventoryService {
  ...
  async getItems(tenantId: string, branchId?: string) {
    return this.prisma.inventoryItem.findMany({
      ...
      include: { supplier: true, products: { select: { id: true, name: true } } },
      orderBy: { name: 'asc' },
    });
  }
  ...
  async recordMovement(tenantId: string, data: {
    ...
    const item = await this.prisma.inventoryItem.findFirst({ where: { id: data.itemId, tenantId } });
    ...
    if (newStock < 0) throw new BadRequestException('Insufficient stock for this movement');
    ...
    const [movement] = await this.prisma.$transaction([
      ...
      this.prisma.inventoryItem.update({ where: { id: data.itemId }, data: { currentStock: newStock } }),
    ]);
  ...
  async getMovements(tenantId: string, itemId?: string) {
    return this.prisma.inventoryMovement.findMany({
      where: { item: { tenantId }, ...(itemId && { itemId }) },
  ...
  async getStockReport(tenantId: string) {
    const items = await this.prisma.inventoryItem.findMany({
      where: { tenantId, isDeleted: false },
      include: { movements: { orderBy: { createdAt: 'desc' }, take: 5 }, supplier: true },
    });

    const totalValue = items.reduce((sum, item) => sum + (Number(item.currentStock) * Number(item.unitCost || 0)), 0);
    const lowStock = items.filter(item => Number(item.currentStock) <= Number(item.minStockLevel));

    return { totalItems: items.length, totalValue, lowStockCount: lowStock.length, lowStockItems: lowStock, items };
  }
}
